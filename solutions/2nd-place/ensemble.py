...
def write_models(blend, file_name, is_top1=False):
    with open(file_name + '.csv', 'w') as f:
        ...
        for id, v in blend.items():
            if is_top1:
                l = ' '.join(
                    ['{}'.format(id_name_label[int(t[0])]) for i_t, t in enumerate(v.most_common(20)) if i_t < 1])
                l += ' None None None None'
                print(l)
                f.write(','.join([str(id), l + '\n']))
            else:
                l = ' '.join(
                    ['{}'.format(id_name_label[int(t[0])]) for i_t, t in enumerate(v.most_common(20)) if i_t < 5])
                f.write(','.join([str(id), l + '\n']))
